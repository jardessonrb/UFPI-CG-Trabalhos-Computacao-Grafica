 ...
 */
GLManimation *
glmLoadAnimation(string pathname, int id, int keyframes, bool face_normals, bool unitize) {
    GLManimation * animation;
    vector<GLMmodel *> models;
    GLMmodel * new_model;
    string path, name, ext;
    std::ostringstream i_to;
    
    animation = new GLManimation;
    string::size_type found = pathname.rfind(".obj"); //Verifica se no pathname tem a extensao ".obj"
    if (found == string::npos && keyframes > 1){ //nao encotrou a extensao ".obj" no pathname. Carrega os frames da animacao
        path = pathname.substr( 0, pathname.rfind('/') + 1);
        name = pathname.substr(pathname.rfind('/') + 1, pathname.length() - (pathname.rfind('/') + 1));
		i_to.str(""); //limpa o buffer com um string vazia
		i_to << 1; //converte o inteiro para uma string no buffer
        new_model = glmReadOBJ((char *) (path + name + "/" + name + i_to.str() + ".obj").c_str());
        if (unitize) glmUnitize(new_model);
        if (face_normals) glmFacetNormals(new_model);
        glmVertexNormals(new_model, 90.0);
        models.push_back(new_model);
        
        for (int i = 2; i <= keyframes; i++) {
			i_to.str(""); 
			i_to << i; 
			new_model = glmReadOBJ((char *) (path + name + "/" + name + i_to.str() + ".obj").c_str(), false);
            new_model->materials = models[0]->materials;
            new_model->groups = models[0]->groups;
            if (unitize) glmUnitize(new_model);
            if (face_normals) glmFacetNormals(new_model);
            glmVertexNormals(new_model, 90.0);
            models.push_back(new_model);
    ...
    if(keyframes < 2 && found != string::npos){ //encontrou a extensao ".obj". Carrega apenas um arquivo.
        name = pathname.substr(pathname.rfind('/') + 1, pathname.length() - (pathname.rfind('/') + 1)-4);
        new_model = glmReadOBJ((char *) pathname.c_str());
        if (unitize) glmUnitize(new_model);
        if (face_normals) glmFacetNormals(new_model);
        glmVertexNormals(new_model, 90.0);
        models.push_back(new_model);
 ...
 */
GLvoid glmDrawAnimation(GLManimation * animation, int keyframe, GLuint mode){
    if (keyframe < 0) keyframe = 0;
    if (keyframe >= animation->keyframes) keyframe = animation->keyframes - 1;
    if (!animation->models.empty()) glmDraw(animation->models[keyframe], mode);
}
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    for (i = 0; i < nummaterials; i++) {
        ...
        model->materials[i].emissive[1] = 0.0;
        model->materials[i].emissive[2] = 0.0;
        model->materials[i].emissive[3] = 0.0;
        model->materials[i].texture = 0;
    }
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'K':
            ...
            case 'm':
                ...
                std::string file = buf;
                file = path + file;
                GLubyte * texture_data = NULL; // Armazena a imagem da textura
                int texture_width, texture_height; //Largura e altura da textura
                texture_data = glmReadPPM((char *)file.c_str(), &texture_width, &texture_height);
                if(texture_data){
                    //Gera nomes identificadores (IDs) para as texturas
                    glGenTextures(1, &model->materials[nummaterials].texture);
                    glBindTexture(GL_TEXTURE_2D, model->materials[nummaterials].texture);
                    //Definicao dos parametros da textura
                    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texture_width, texture_height, 0, GL_RGB, GL_UNSIGNED_BYTE, texture_data);
                    // glGenerateMipmap(GL_TEXTURE_2D); //nao esta definda no opengl usada no falcon
 ...
 */
static GLvoid
glmFirstPass(GLMmodel* model, FILE* file, bool readmaterials)
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'v':               /* v, vn, vt */
            ...
            case 'm':
                ...
                if(readmaterials) glmReadMTL(model, buf);
                break;
            case 'u':
                /* eat up rest of line */
                fgets(buf, sizeof(buf), file);
 ...
 */
static GLvoid
glmSecondPass(GLMmodel* model, FILE* file, bool readmaterial)
{
    GLuint numvertices;        /* number of vertices in model */
    GLuint numnormals;         /* number of normals in model */
    GLuint numtexcoords;       /* number of texcoords in model */
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'v':               /* v, vn, vt */
            ...
            case 'u':
                fgets(buf, sizeof(buf), file);
                sscanf(buf, "%s %s", buf, buf);
                if(readmaterial) group->material = material = glmFindMaterial(model, buf);
                break;
            case 'g':               /* group */
                /* eat up rest of line */
                fgets(buf, sizeof(buf), file);
 ...
 */
GLMmodel* 
glmReadOBJ(char* filename, bool readmaterials)
{
    GLMmodel* model;
    FILE* file;
    ...
    glmFirstPass(model, file, readmaterials);
    ...
    glmSecondPass(model, file, readmaterials);
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group) {
        ...
        if (mode & GLM_TEXTURE){
            glBindTexture(GL_TEXTURE_2D, model->materials[group->material].texture);
        }
